...
class page {
	...
	public $ads = false;		// display google ads in menu
	public $logo = false;		// display logo
	public $social = false; 	// display social bookmarking button in footer
	...
	function set_logo($x) { $this->logo = ($x === true) ? true : false; }
	function get_logo() { return $this->logo; }
	
	function displayHead($styles = false, $header = false) {
		$head_title = 'Experimentum' . ' &rArr; ' . implode(' &rArr; ', $this->title);
?><!DOCTYPE html>

<html xmlns:fb="http://www.facebook.com/2008/fbml" lang="<?= $this->language ?>">
<head>
	...
	<meta name="verify-v1" content="<?= GOOGLE_VALIDATION ?>" /> 
	<meta property="og:site_name" content="Experimentum"/>
	...
	<link rel="alternate" type="application/rss+xml" title="Experimentum via RSS" href="/rss" />
...
<?php
	...
	function displayBody() {
		...
		$headertitle = array('<li><a href="/">Experimentum</a></li>');
		foreach ($this->title as $link => $t) {
			if (!empty($t)) {
				if (is_numeric($link)) $headertitle[] = "<li>&rArr; $t</li>";
				else $headertitle[] = "<li>&rArr; <a href='$link'>$t</a></li>";
			}
		}
...
<?php
	...
	function displayFooter() {
		...
		if ($this->menu) {
			...
			$menu .= '	</ul>' . ENDLINE;
			if (!MOBILE && strpos($_SERVER['PHP_SELF'], '/res/') === false) {
				// add facebook integration for non-researcher pages
				//$menu .= '	<div id="fb_facepile"><fb:facepile max-rows="1" width="200"></fb:facepile></div>' . ENDLINE;
			}
...
<div id='loginbox' class="modal" title='Login to Experimentum'>
...
<div id='guestloginbox' class="modal" title='Login to FaceResearch as a Guest'>
...
<footer id="footer">
	<!--<small>Contact us at <a href="mailto:email">email</a></small>-->
	<small>This website uses cookies to allow you to log in. Continued use of the website implies you agree to this.</small>

	<!--<small class='users_online'><?= sprintf(loc("%d users currently online"), getOnlineUsers()) ?></small>-->
</footer> 
