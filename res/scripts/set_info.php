...
if (validID($_GET['id']) && !permit('sets', $_GET['id'])) {
    $return['error'] = "You are not authorised to access this set.";
    scriptReturn($return);
    exit;
}
...
if ($myset->get_num_rows() == 0) { 
    $return['error'] = "The set does not exist.";
    scriptReturn($return);
    exit;
}
...
if (in_array($_SESSION['status'], array('admin', 'super', 'res'))) {
    ...
    $status .= '<button class="tinybutton" id="all-status-change">Set all component statuses to set status</button>';
} else {
...
$return['items'] = generate_set($item_id);
