...
foreach ($questions as $n => $q) {
    ...
    switch ($q['type']) {
        ...
        case 'input':
        case 'text':
            $input->set_maxlength($q['maxlength']);
            break;
        case 'time': break;
        case 'textarea': break;
...
<div id="excel_input">
    ...
    <p>Question types for mixed questionnaires are: <kbd>select</kbd>, 
        ...
        <kbd>datemenu</kbd>, <kbd>time</kbd>, <kbd>text</kbd>, and <kbd>msgRow</kbd>. Question types for 
...
<script>
    ...
    function changeType(qid) {
        ...
        if (theType == 'select') { 
            ...
            theInputCell.empty().append(newInput);
            ...
            newInput = $('<ul class="radio" />').attr('id', newID);
            ...
            if (theInput.is('select:not(.selectnum)')) {
                ...
                theInput.find('option[value!="NULL"]').each( function(i) {
                    ...
                    $('<input />') .attr('type', 'radio')
                                    .attr('name', newID)
                                    .attr('id', newID + '_' + $(this).attr('value'))
                                    .val($(this).attr('value'))
                                    .appendTo(newLi);
                    $('<label />').html($(this).html())
                                    .attr('for', newID + '_' + $(this).attr('value'))
                                    .appendTo(newLi);
            ...
            newInput = $('<table class="slider" />');
            var ra = newInput.append('<tbody />').find('tbody').append('<tr />').find('tr');
            $('<td />').addClass('anchor').html('<span class="editText" id="low_anchor_'+qid+'">low anchor</span>').appendTo(ra);
            
            $slider = $("<div class='slider' />").attr('id', 'q' + qid).slider({
                min: 0,
                max: 100,
                step: 1
            });
            ...
            // !        Change to time
            newInput = $('<div class="time" />').attr('id', 'q' + qid);
            
            newInput_hour = $('<select />')
                            .attr('id', 'q' + qid + "_hour")
                            .attr('name', 'q' + qid + "_hour");
            $('<option />').attr('value', 'NULL').html("HOUR").appendTo(newInput_hour);
            for (h=0; h < 24; h++) {
                hpad = (h+'').length == 1 ? '0' + h : h;
                $('<option />').attr('value', h).html(hpad).appendTo(newInput_hour);
            }
            newInput_min = $('<select />')
                            .attr('id', 'q' + qid + "_min")
                            .attr('name', 'q' + qid + "_min");
            $('<option />').attr('value', 'NULL').html("MIN").appendTo(newInput_min);
            for (m=0; m < 60; m++) {
                mpad = (m+'').length == 1 ? '0' + m : m;
                $('<option />').attr('value', m).html(mpad).appendTo(newInput_min);
            }
            newInput.append(newInput_hour).append(":").append(newInput_min);
            theInputCell.empty().append(newInput);
        } else if (theType == 'msgRow') {
            // !        Change to msgRow
            newInput = '';
            theInputCell.empty().append(newInput);
